...
Generator.prototype.generateMapping = function(schema, cb) {
    var cleanTree = getCleanTree(JSON.parse(JSON.stringify(schema.tree)), schema.paths, '');
...
// @return the tree
//
function getCleanTree(tree, paths, prefix, nested) {
        ...
        if (type) {
            ...
            if (value[0]) {
                ...
                if (paths[field].schema && paths[field].schema.tree && paths[field].schema.paths) {
                    cleanTree[field] = getCleanTree(paths[field].schema.tree, paths[field].schema.paths, '', field);
                }
