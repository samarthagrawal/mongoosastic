...
module.exports = function elasticSearchPlugin(schema, options){
  ...
  schema.statics.river = function(cb) {
    ...
    if (!(useRiver && useRiver.db) && !this.db.name) throw "ERROR: useRiver.db <empty> or model.index call before mongoose.connect on model: " + model.modelName
    esClient.putRiver(
      'mongodb', 
      (useRiver && useRiver.collection) ? useRiver.collection : indexName, 
      {
        type: 'mongodb',
        mongodb: { 
          db: (useRiver && useRiver.db) ? useRiver.db : this.db.name,
          collection: (useRiver && useRiver.collection) ? useRiver.collection : indexName,
  ...
  schema.methods.index = function(index, type, cb){
    ...
    var model = this;
    setIndexNameIfUnset(model.constructor.modelName);
    !useRiver && esClient.index(index || indexName, type || typeName, serialize(model, indexedFields), {id:model._id.toString()}, cb);
  }
  /**
  ...
  schema.statics.search = function(query, options, cb){
    ...
    esClient.search(query, function(err, results, res){
      if(err){
        ...
        if (alwaysHydrate || options.hydrate || options.hydrateWithScore) {
          hydrate(results, model, cb, options.hydrateWithScore);
        }else{
          cb(null, res);
        }
      }
...
function hydrate(results, model, cb, withScore){
  var resultsMap = {}
  if (withScore) {
    var scoreMap = {}
  }
